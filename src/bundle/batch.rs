...
#[derive(Debug)]
pub struct BatchValidator<'vk, R: RngCore + CryptoRng> {
    proofs: Option<Result<plonk::BatchVerifier<'vk, vesta::Affine, R>, plonk::Error>>,
    signatures: Vec<BundleSignature>,
    vk: &'vk VerifyingKey,
}

impl<'vk, R: RngCore + CryptoRng> BatchValidator<'vk, R> {
    /// Constructs a new batch validation context.
    pub fn new(vk: &'vk VerifyingKey, rng: R) -> Self {
        BatchValidator {
            proofs: Some(Ok(plonk::BatchVerifier::new(&vk.params, rng))),
            signatures: vec![],
            vk,
        }
    }
    ...
    pub fn add_bundle<V: Copy + Into<i64>>(
        ...
        self.proofs = self.proofs.take().map(|strategy| {
            strategy.and_then(|strategy| {
                bundle
                    .authorization()
                    .proof()
                    .verify_with_strategy(self.vk, &bundle.to_instances(), strategy)
                    .map_err(|e| {
                        debug!("Per-proof component of batch validation failed: {}", e);
                        e
                    })
            })
        });
    ...
    pub fn validate(self) -> bool {
        let rng = match self
            .proofs
            .unwrap()
            .map(|strategy| strategy.finalize_and_return_rng())
            .unwrap_or(None)
        {
            Some(rng) => rng,
            None => {
                debug!("Proof batch validation failed");
                return false;
            }
        };
